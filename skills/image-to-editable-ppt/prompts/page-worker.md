...
- <skill root>/references/image-backend-integration.md
- <skill root>/references/workflow-contract.md
- <skill root>/references/manifest-schema.md
- <skill root>/references/qa-rubric.md
- <skill root>/references/cli-contracts.md

如果这是 repair mode，prompt 还会包含 repair item id、失败证据和允许修改范围。只修复 repair item 指定的最小失败范围；不要重建整页，除非 repair item 明确说明整页 manifest 不可用。

在任何生图或改图前，使用 `page_request.json.image_backend` 指定的 image backend。遵守 `page_request.json.user_requirements_and_feedback`（如果存在）。不要处理、修改或重新生成已确认 sample page。如果指定 backend 不可用，返回 blocker。

目标：
把 source page 重建成对象级可编辑 PowerPoint。所有可读文字必须是原生 PPT text box。只有基础 primitive 可以是 native PPT shape。图标、pictogram、手绘对象、装饰标记、风格承载对象，以及无法确定的非文字对象，必须使用 confirmed image backend 生成/编辑为独立资产。
...
3. 背景策略：纯色/规则背景用 native PPT 对象或确定性 runtime；复杂照片/空间/仪表盘需要 image backend 时，必须把 source 当作 edit target 和强约束参考。可以做整张 clean base，但结果必须对照 source 保留构图、透视、主要物体位置、色彩、光照和背景身份；不允许生成一个相似但不同的新背景。
...
使用 confirmed image backend 生成 clean base、背景修复、asset sheet 和 repair asset。不要创建一次性 SDK runner。不要使用临时代码、SVG、canvas、HTML/CSS 或 Python 绘图来伪造复杂视觉资产。确定性 CLI/runtime 只可用于归一化、记录、去底、切分、裁剪、构建、验证和 QA。
...
source-derived raster asset 必须留安全边距并检查 alpha 边缘。不要手写临时裁剪代码；用 `editppt image crop <page_dir> --source source.png --box <left,top,right,bottom> --out assets/<name>.png --source-type source-derived-rasterization --padding <n> --remove-border-bg` 生成。裁出的可见像素贴到图片边缘时，只能判为“可能裁断”：需要对照 source/contact sheet 判断是否扩大 crop；只有在 manifest 对该资产显式设置 `require_edge_safe_alpha: true` 时，validation 才把贴边作为硬失败。
...
repair mode 还必须：
- 对照 repair item 和 evidence path 说明修复了什么。
- 只改允许修改范围内的对象、asset 或 manifest section。
- 保留未涉及区域，不做无关重建或重排。
- 如果 repair item 无法执行，返回 blocker，并说明 evidence path 和未完成原因。
