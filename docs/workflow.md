...
1. **创建任务目录并归一化输入**：创建独立任务目录，把输入（图片/PDF/图片版 PPT）归一化为 `pages/page_NNN/source.png`，并写入默认 `editppt image` backend。
...
- 如果缺少图片生成/编辑能力，skill 仍会先交付当前可打开、结构有效的 PPT，并在验证结果里说明缺失资产。
